...
# Vrtrix默认IP
default_vrtrix_ip = "127.0.0.1"
# Vrtrix默认端口
default_vrtrix_port = 13000

# CMCapture默认IP
default_listener_ip = "127.0.0.1"
# CMCapture默认端口
default_listener_port = 13001

class AddVrtrixWidget(BaseDeviceWidget):
    def __init__(self, settings):
        super(AddVrtrixWidget, self).__init__(settings)
        form_layout = QtWidgets.QFormLayout()

        self.setStyleSheet("Label: { color: black}")
        self.setWindowTitle(self.tr("Add Vrtrix Client"))

        msg = self.tr('<P>Vrtrix is a great motion capture program!</P>')
        self.set_info(msg)

        self.name = QtWidgetFactory.create_QLineEdit(settings.value("VrtrixName", "Vrtrix"))
        form_layout.addRow(self.tr("Name"), self.name)

        self.device_ip = QtWidgetFactory.create_QLineEdit_IP(settings.value("VrtrixIp", default_vrtrix_ip))
    ...
    def update_device(self, device):
        ...
        try:
            ...
            QMessageBox.warning(self, self.tr("Error"), self.tr("Invalid port"))
    ...
    def do_add(self):
        ...
        self.settings.setValue("VrtrixName", device_name_text)
        self.settings.setValue("VrtrixIp", device_ip_text)
        self.settings.setValue("VrtrixPort", device_port_text)
        self.settings.setValue("VrtrixListenIp", listen_ip_text)
        self.settings.setValue("VrtrixListenPort", listen_port_text)

        return True


class VrtrixDevice(PeelDeviceBase):

    def __init__(self,
                 name,
                 device_ip=default_vrtrix_ip,
                 device_port=default_vrtrix_port,
                 listen_ip=default_listener_ip,
                 listen_port=default_listener_port,
                 takes=None):

        super(VrtrixDevice, self).__init__(name)
        ...
        self.downloadFileList = None

        if takes is None:
            self.takes = {}
        else:
            self.takes = takes
        self.current_takename = None
    ...
    def start_services(self):
        ...
        if self.thread is not None and self.server is not None:
            print("Stopping current Vrtrix OSC Server")
        ...
        self.client.send_message(common.cmd_rep_heatbeat, (self.listen_ip, self.listen_port))

        # init status is offline
        self.state = "OFFLINE"
    ...
    def command(self, command, arg):
        ...
        if command == "record":
            ...
            #   参数3：表示导出bvh的路径和名称
            #   参数4：表示token
            self.current_takename = arg
            #self.client.send_message('/control/record', (True, self.listen_ip, self.listen_port, self.current_takename))
            self.client.send_message(common.cmd_req_record, (self.listen_ip, self.listen_port, True,  self.current_takename))

        if command == "stop":
            self.current_takename = arg
            self.client.send_message(common.cmd_req_record, (self.listen_ip, self.listen_port, False, self.current_takename))
    ...
    def callback(self, address, command, *args):
        ...
        if command == common.cmd_rep_takepath:
            #取得远程设备录制文件名
            files_cnt = len(args)
            if files_cnt != 3:
                return
            ...
            take_name = args[0]
            proj_path = args[1].replace('\\', '/')
            take_fullPath = args[2].replace('\\', '/')
            print("Adding " + str(take_name) + " " + str(take_fullPath))
            if take_name:
                if take_name in self.takes:
                     remote_files = self.takes[take_name]['remote_files']
                     if remote_files:
                         remote_files.append(take_fullPath)
                     else:
                         self.takes[take_name]['remote_files'] = [take_fullPath]
                else:
                    self.takes[take_name] = { 'remote_project': proj_path, 'remote_files' : [take_fullPath], 'local_files': [] }

            print(f"record take fullpath {args[0]}, {args[1]}")
            return
    ...
    def harvest(self, directory):
        thread = VrtrixDownloadThread(self, directory)
        for take, remote_item in self.takes.items():
            ...
            local_files.clear()
            # file_ext = os.path.splitext(remote_file)[-1]
            # thread.files.append(FileItem(remote_file, take + file_ext, remote_project))

            for remote_file in remote_files:
                #file_ext = os.path.splitext(remote_file)[-1]
                #thread.files.append(FileItem(remote_file, take + file_ext))
    ...
    @staticmethod
    def dialog(settings):
        return AddVrtrixWidget(settings)
    ...
    def dialog_callback(widget):
        ...
        try:
            ...
            return VrtrixDevice(name, device_ip, device_port, listen_ip, listen_port)
        except ValueError:
...
class VrtrixDownloadThread(DownloadThread):

    def __init__(self, device, directory, listen_port=8444, takes=None):
        super(VrtrixDownloadThread, self).__init__()
        self.takes = takes
        self.device = device
        self.listen_port = listen_port
    ...
    def teardown(self):
        if self.socket:
            self.socket.close()
            self.socket = None
        super(VrtrixDownloadThread, self).teardown()

    def run(self):

        print("Downloading %d Vrtrix files" % len(self.files))
        ...
        try:
            ...
            while self.is_running():
                ...
                # # Get remote project name
                # remote_project_name = os.path.basename(this_file.remote_project)
                # remote_project_root = this_file.remote_project.replace(remote_project_name, '')
                # relative_path = this_file.remote_file.replace(remote_project_root, '')
                # if relative_path.startswith('/') == False and relative_path.startswith('\\') == False:
                #     relative_path = '/' + relative_path
                    

                # full_path = self.directory + relative_path
                # local_folder = os.path.dirname(os.path.abspath(full_path))
                ...
                # Tell the CMTracker we want it to send us a file
                self.device.client.send_message(common.cmd_req_transport, (self.device.listen_ip, self.listen_port, this_file.remote_file))
