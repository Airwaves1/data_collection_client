...
# CMTracker默认端口
default_tracker_port = 12000
# RemoteTool默认端口
defualt_remotetool_port = 12500

# CMCapture默认IP
default_listener_ip = "127.0.0.1"
# CMCapture默认端口
default_listener_port = 12001

class AddTrackerWidget(BaseDeviceWidget):
    def __init__(self, settings):
        super(AddTrackerWidget, self).__init__(settings)
        form_layout = QtWidgets.QFormLayout()

        self.setStyleSheet("Label: { color: black}")
        self.setWindowTitle(self.tr("Add CMTracker Client"))

        msg = self.tr('<P>CMTracker is a great motion capture program!</P>')
        self.set_info(msg)

        self.name = QtWidgetFactory.create_QLineEdit(settings.value("CMTrackerName", "CMTracker"))
        form_layout.addRow(self.tr("Name"), self.name)

        # IP Address
        self.device_ip = QtWidgetFactory.create_QLineEdit_IP(settings.value("CMTrackerIp", default_tracker_ip))
        form_layout.addRow(self.tr("Address"), self.device_ip)

        self.device_port = QtWidgetFactory.create_QLineEdit_port(settings.value("CMTrackerPort", str(default_tracker_port)))
    ...
    def do_add(self):
        ...
        # save it to register
        self.settings.setValue("CMTrackerName", device_name_text)
        self.settings.setValue("CMTrackerIp", device_ip_text)
        self.settings.setValue("CMTrackerPort", device_port_text)
        self.settings.setValue("CMTrackerRemoteToolPort", remotetool_port_text)
        self.settings.setValue("CMTrackerListenIp", listen_ip_text)
        self.settings.setValue("CMTrackerListenPort", listen_port_text)

        return True


class CMTracker(PeelDeviceBase):

    def __init__(self,
                 name,
                 device_ip,
                 device_port=default_tracker_port,
                 remotetool_port = defualt_remotetool_port,
                 listen_ip=default_listener_ip,
                 listen_port=default_listener_port,
                 takes=None):

        super(CMTracker, self).__init__(name)
    ...
    def start_services(self):
        ...
        if self.thread is not None and self.server is not None:
            print("Stopping current CMTracker OSC Server")
        ...
        print("Starting CMTracker thread")
    ...
    def callback(self, address, command, *args):
        ...
        if command == common.cmd_rep_record_stop:
            ...
            # 所有文件数量
            file_cnt = int(args[1])
            human_file_cnt = int(args[2])
            remote_files = []
            skeleton_index = []
            file_list_start = 3
            file_list_end = file_list_start + file_cnt
            if file_cnt > 0:
                remote_files = args[file_list_start : file_list_end]

            if human_file_cnt > 0:
                skeleton_index = args[file_list_end: file_list_end + human_file_cnt]

            if self.current_take:
                self.takes[self.current_take] = { 'remote_project': proj_fullPath, 'remote_files' : remote_files, 'local_files': [], 'skeleton_index': skeleton_index}
    ...
    def harvest(self, directory):
        thread = CMTrackerDownloadThread(self, directory)
    ...
    @staticmethod
    def dialog(settings):
        return AddTrackerWidget(settings)
    ...
    def dialog_callback(widget):
        ...
        try:
            ...
            return CMTracker(name, device_ip, device_port, remotetool_port, listen_ip, listen_port)
        except ValueError:
...
class CMTrackerDownloadThread(DownloadThread):

    def __init__(self, device, directory, listen_port=8444, takes=None):
        super(CMTrackerDownloadThread, self).__init__()
        self.takes = takes
        self.device = device
        self.listen_port = listen_port
    ...
    def teardown(self):
        if self.socket:
            self.socket.close()
        super(CMTrackerDownloadThread, self).teardown()

    def run(self):

        print("Downloading %d CMTracker files" % len(self.files))
